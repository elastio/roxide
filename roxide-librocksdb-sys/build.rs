...
fn build_rocksdb() {
    ...
    // Only write this file if it didn't already exist, so the compiler doens't try to re-build all
    // the time
    if !cpp_defines_path.exists() {
        let mut cpp_defines = fs::File::create(&cpp_defines_path).unwrap();
        writeln!(
            &mut cpp_defines,
            "// Auto-generated by roxide-librocksdb-sys/build.rs"
        )
        .unwrap();
        writeln!(&mut cpp_defines, "#pragma once").unwrap();
        for (name, value) in defines {
            if let Some(value) = value {
                writeln!(&mut cpp_defines, "#define {} {}", name, value).unwrap();
            } else {
                writeln!(&mut cpp_defines, "#define {}", name).unwrap();
            }
        }
    }
